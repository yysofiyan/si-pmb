...
<div class="article">
	...
	<div style=" padding:1px; background-color:#CCCCCC; font-size:11px">
				<table width="1700" border="0" cellspacing="0" cellpadding="3px">
				  <form id="caricari" action="daftar_calon_terdaftar_kesehatan.php?stat=<?php echo"$stat"; ?>" method="POST"> 
				  <tr bgcolor="#FFFFFF" valign="middle">
					...
					<td width="11%"><input name="carinya" type="text" size="25" value="<?php echo"$carinya"; ?>" maxlength="30" /></td>
    ...
    <?php if($pilihan!=0 && $jurusan!=0) { ?>
				<table width="1700" border="0" cellspacing="1" cellpadding="3px">
				  ...
				  ?>
					<td width="2%">No</td>
					<td width="6%"><b>Test Kesehatan</b></td>
					<td width="4%"><b>Test Tulis</b></td>
				...
				$qview1 = "SELECT a.nomor_pendaftaran, a.nama_lengkap, a.tinggi_badan, a.berat_badan, a.tempat_lahir, 
						   ...
						   a.cek_nilai_lolos_1 as lolos_stat, a.cek_kesehatan
                    		...
                    		WHERE (a.nomor_pendaftaran LIKE '%" . $carinya . "%' OR a.nama_lengkap LIKE '%" . $carinya . "%')
								  ...
								  AND a.cek_tinggi=1 AND a.cek_syarat=1 AND a.cek_pembayaran=1 AND a.cek_nilai>0 AND a.cek_nilai_lolos_1=1
				...
				$qview1 = "SELECT a.nomor_pendaftaran, a.nama_lengkap, a.tinggi_badan, a.berat_badan, a.tempat_lahir, 
						   ...
						   a.kewarganegaraan, a.jurusan_1, a.jurusan_2, a.tempat_test, a.password, a.password_temp,
						   a.waktu_daftar, a.kode_setting, b.jurusan, a.cek_tinggi, a.cek_nilai, a.cek_syarat, a.cek_pembayaran,
						   a.cek_nilai_lolos_2 as lolos_stat, a.cek_kesehatan
                    		FROM tb_pendaftaran as a, jurusan as b
                    		WHERE (a.nomor_pendaftaran LIKE '%" . $carinya . "%' OR a.nama_lengkap LIKE '%" . $carinya . "%')
								  AND a.kode_setting='$dtahun[0]' AND a.jurusan_2=b.kode_jurusan AND a.jurusan_2='$jurusan'
								  AND a.cek_tinggi=1 AND a.cek_syarat=1 AND a.cek_pembayaran=1 AND a.cek_nilai>0 AND a.cek_nilai_lolos_2=1
                    		ORDER BY a.cek_nilai DESC, a.waktu_daftar ASC";
				}
				...
				?>
				  <tr bgcolor="#FFFFFF">
				    ...
				    <td style="vertical-align:middle;"><?php if($dview1['cek_kesehatan']==1) { echo "Lolos"; } else { echo "Tidak Lolos"; } ?> 
                    <span style="margin-right:10px;text-align:right; float:right; margin-top:2px;">
                    <?php
					$nopen=$dview1['nomor_pendaftaran'];
					if($dview1['cek_kesehatan']==1) { echo "<a href='daftar_calon_terdaftar_kesehatan_save.php?stat=0&id=$nopen&pilihan=$pilihan&jurusan=$jurusan'><img src='../images/pass.png' width='16' title='Unpass'></a>"; }
					else  { echo "<a href='daftar_calon_terdaftar_kesehatan_save.php?stat=1&id=$nopen&pilihan=$pilihan&jurusan=$jurusan'><img src='../images/pass_un.png' width='16' title='Pass'></a>"; }
                    ?>
                    </span>
                    </td>
